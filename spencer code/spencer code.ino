...
//THESE TWO VARIABLES ARE DEFINED AS PART OF A TEST FOR AUTONOMY, LEAVE THEM UNLESS PROVEN BAD
#if defined (OV2640_MINI_2MP_PLUS)
 ...
 * 
 * 
 * A lot of the following lines are simply set up for the code to recognize what type of software and module you have. Should not need to mess with any of it.
 * 
 * 
 * 
 */
...
//After this point begins the actual looping code, where it will repeatedly check the camera mode, and wait for commands from the host app
//we eventually want to take out the waiting for host app stuff
//A lot of it is redundant checking, since we will always be using the same camera. Needs to be cleaned up
void loop() {
  ...
  //Here is where the actual photo taking shenanigans start
  if (mode == 1)
  {
    ...
    if (currentTime % 10000 == 0) //if (start_capture == 1)
    {
      myCAM.flush_fifo();
      myCAM.clear_fifo_flag();
      //Start capture
  ...
  //we DO NOT want mode 2. When the file is cleaned, erase this section
  else if (mode == 2)
  {
    while (1)
    {
...
//This is where the arduino receives the photo from the camera, and processes it
//somewhere in here is where it is saved as well, but unsure where
uint8_t read_fifo_burst(ArduCAM myCAM)
{
